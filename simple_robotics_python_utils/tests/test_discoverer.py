#!/usr/bin/env python3
from simple_robotics_python_utils.pubsub.discoverer import Discoverer, DiscovererTypes, UDP_BROADCAST_TIMEOUT
    ...
    def test_discoverer(self):
        ...
        for i in range(reader_num):
            readers_processes.append(
                Process(
                    target=reader_worker, args=(recognized_writers, )                    
                )
            )
