...
#define getcwd _getcwd
  ...
  if (g_stderr_verbosity >= Verbosity_INFO) {
    // Rico: commenting the header out, because it's annoying :/
    ...
    // preamble_explain); 	} else { 		fprintf(stderr, "%s\n", preamble_explain);
    // 	}
    // }
    fflush(stderr);
  }
  // Rico: commenting these header out as well, because they are annoying :/
  // VLOG_F(g_internal_verbosity, "arguments: " LOGURU_FMT(s) "",
  // s_arguments.c_str()); if (strlen(s_current_dir) != 0)
  // {
  // 	VLOG_F(g_internal_verbosity, "Current dir: " LOGURU_FMT(s) "",
  // s_current_dir);
  // }
  // VLOG_F(g_internal_verbosity, "stderr verbosity: " LOGURU_FMT(d) "",
  // g_stderr_verbosity); VLOG_F(g_internal_verbosity,
  // "-----------------------------------");
...
static void print_preamble(char *out_buff, size_t out_buff_size,
  ...
  if (g_preamble_time && pos < out_buff_size) {
    int bytes =
        snprintf(out_buff + pos, out_buff_size - pos, "%02d:%02d:%02d.%03lld ",
                 time_info.tm_hour, time_info.tm_min, time_info.tm_sec,
                 ms_since_epoch % 1000);
    if (bytes > 0) {
      pos += bytes;
