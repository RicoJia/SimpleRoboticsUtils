...
TEST(CvUtilsTest, TestFrobeniusNorm) {
    ...
    m << 1.0, 2.0, 3.0,
        0.0, 0.0, 0.0,
        1.0, 2.0, 0.0;
    double norm = SimpleRoboticsCppUtils::getFrobeniusNorm(m);
...
TEST(CvUtilsTest, TestPixel2cam) {
    ...
    cv::Point2f result = SimpleRoboticsCppUtils::pixel2cam(p, K);
...
TEST(CvUtilsTest, TestRT2EigenIsometry3d) {
    ...
    cv::Mat R = (cv::Mat_<double>(3, 3) << 0, -1, 0,
                                           1, 0, 0,
                                           0, 0, 1);
    cv::Mat t = (cv::Mat_<double>(3, 1) << 1, 0, 0);
    auto trans = SimpleRoboticsCppUtils::cv_R_t_to_eigen_isometry3d(R, t);
...
TEST(CvUtilsTest, TestInvertMat){
    cv::Mat R = (cv::Mat_<double>(3, 3) << 0, -1, 0,
                                           1, 0, 0,
                                           0, 0, 1);
    cv::Mat R_inv_expected = (cv::Mat_<double>(3, 3) << 
                                           0, 1, 0,
                                           -1, 0, 0,
                                            0, 0, 1);
    cv::Mat R_inv;
    bool succeeded = SimpleRoboticsCppUtils::invert_mat(R, R_inv);
...
TEST(CvUtilsTest, TestSkewMatrixConversion){
    ...
    cv::Mat expected = (cv::Mat_<double>(3, 3) << 
                        0, -3, 2,
                        3, 0, -1,
                        -2, 1, 0);
    // Call the function
    cv::Mat result = SimpleRoboticsCppUtils::skew_symmetric_mat(t);
